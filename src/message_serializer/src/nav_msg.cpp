...
using namespace std;
// Output Indices 
#define LEFT_JOY1 	0 // The left joystick value will be assigned in the first position in the joystick output array.
#define LEFT_JOY2       1
#define LEFT_JOY3       2
#define RIGHT_JOY1	3
#define RIGHT_JOY2      4
...
unsigned char message_sending[6] = {127,127,127,127,127,127};
...
void joystickSaver(const sensor_msgs::Joy::ConstPtr& joy){
 ...
 if (right_joy < 0){
  ...
  message_sending[LEFT_JOY1] = l_j;
  message_sending[LEFT_JOY2] = l_j;
  message_sending[LEFT_JOY3] = l_j;
  message_sending[RIGHT_JOY1] = r_j;
  message_sending[RIGHT_JOY2] = r_j;
  message_sending[RIGHT_JOY3] = r_j;
  /*
  for (int i=2;i<6;i++){
  message_sending[i] = 0;
  }
...
/* "push_back" only puts one value at a time into the data array in "UInt8MultiArray. The first term entered will be the last term in the array.[3] */
      ...
      /* joyoutput.data.insert(joyoutput.data.end(),message_sending.begin(),message_sending.end() );*/
